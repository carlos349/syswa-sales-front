<template>
    <div>
        <base-header class="header pb-4 pt-2 pt-lg-4 d-flex align-items-center" style="min-height: 50px; background-image: url(img/theme/inventario.jpg); background-size: cover; background-position: center top;">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <base-button class="float-right mt-7 mr-2" size="sm" type="primary" @click="modals.modal7 = true">
        ...
        <a-modal v-model="modals.modal7" width="60%" :footer="false" title="Gestión de sucursales" :closable="true" >
            <template>
                <div class="row">
                    <div class="col-4">
                        <a-select placeholder="Seleccione sucursal" class="w-100" @change="selectBranch">
                            <template v-for="branch of branches">
                                <a-select-option :key="branch._id" v-if="branch.active" :value="branch._id">
                                    {{branch.name}}
                                </a-select-option>
                            </template>
                        </a-select>
                    ...
                    <div class="col-1"></div>
                    <div class="col-4">
                        <a-select v-if="branchSelect" allowClear placeholder="Seleccione sucursal" class="w-100 reloadSelect">
                            <template v-for="product of products">
                                <a-select-option :key="product._id" v-if="product.readyIn" @click="selectProduct(product)" :value="product._id">
                                    {{product.product}}
                                </a-select-option>
                            </template>
                        </a-select>
                    ...
                    <div class="col-3">
                        <base-button @click="addProduct" size="sm" type="success">
                            <a-icon type="form" class="mr-2" style="vertical-align:1px;font-size:1.2em;" />
                            Ingresar producto
                        </base-button>
                    </div>
                ...
                <div class="container-fluid mt-2 px-0">
                    <a-table :columns="columnsProductsByBranch" :data-source="productsBranch" :pagination="false" :scroll="getScreen">
                        <template slot="actions" class="mx-auto" slot-scope="record, column">
                            <base-button size="sm" class="float-right" type="success" @click="addToProductBranch(column._id, column.idInventory, column.entry, column.stock)" icon="fa fa-plus"></base-button>
                            <base-button size="sm" class="float-right ml-1" type="danger" @click="removeProductsBranch(column._id, column.idInventory, column.entry)" icon="fa fa-minus"></base-button>
                            <a-input
                                :placeholder="'Cantidad'"
                                class="ml-auto float-right w-50"
                                v-model="column.entry"/>
                        </template>
...
export default {
    ...
    methods: {
        ...
        async selectBranch(value){
            ...
            if (getProducts.data.status == 'ok') {
                this.productsBranch = getProducts.data.data
                for (const product of this.products) {
                    for (const productBranch of this.productsBranch) {
                        productBranch.entry = ''
                        if (product.product == productBranch.product) {
                            product.readyIn = false
                            productBranch.stock = product.quantity - product.consume
                        }  
                    }
                }
            }else{
        ...
        selectProduct(product){
            ...
            for (const productt of this.products) {
                if (productt.product == product.product) {
                    productt.readyIn = false
                }
            }
        },
        async addProduct(){ 
            try{
                const addProduct = await axios.post(`${endPoint.endpointTarget}/products/addProductToProductBranch`, {
                    product: this.productSelect,
                    branch: this.branch
                }, this.configHeader)
                if (addProduct.data.status == 'ok') {
                    this.selectBranch()
                    $('.reloadSelect .ant-select-selection__clear').click()
                }
            }catch(err){
        ...
        async addToProductBranch(_id, idInventory, entry, stock){
            console.log('ajaa')
            if (stock >= entry) {
                try {
                    const addProduct = await axios.put(`${endPoint.endpointTarget}/products/addProductToBranch/${this.branch}`, {
                        id: _id,
        ...
        async removeProductsBranch(_id, idInventory, entry){
            try {
                const addProduct = await axios.put(`${endPoint.endpointTarget}/products/removeProductToBranch/${this.branch}`, {
                    id: _id,
                    idInventory: idInventory,
                    entry: entry
                }, this.configHeader)
